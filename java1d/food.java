...
import static com.example.jin.java1d.newtabbed.foodlist;
...
public class food extends AppCompatActivity{
    ...
    public void onCreate(@Nullable Bundle savedInstanceState){
        ...
        getSupportActionBar().setIcon(R.drawable.banner2_food);

        Integer index =0;

        LinearLayout layout = findViewById(R.id.foodlinear2);
        LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        for (int i = 0; i < foodlist.length; i++) {
            for (int j=0;j<places.length;j++) {
                if (places[j].equals(foodlist[i])) {
                    index = j;
                    break;
                }
            }
            ...
            newbutton.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    try {
                        ...
                        Intent intenttoactivity = new Intent(food.this, place);
