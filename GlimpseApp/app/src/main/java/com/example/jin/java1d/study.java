...
import static com.example.jin.java1d.newtabbed.studylist;
...
public class study extends AppCompatActivity{
    ...
    public void onCreate(@Nullable Bundle savedInstanceState){
        ...
        getSupportActionBar().setIcon(R.drawable.banner2_study);

        Integer index =0;

        LinearLayout layout = findViewById(R.id.studylinear2);
        LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        for (int i = 0; i < studylist.length; i++) {
            for (int j=0;j<places.length;j++) {
                if (places[j].equals(studylist[i])) {
                    index = j;
                    break;
                }
            }
            ...
            newbutton.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    try {
                        ...
                        Intent intenttoactivity = new Intent(study.this, place);
